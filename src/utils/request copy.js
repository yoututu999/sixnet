...
//根据环境变量获取api地址
let baseURL = JSON.parse(process.env.VITE_BASE_URL) 
...
class HttpRequest {
  ...
  setInterceptors (instance) {
    ...
    instance.interceptors.request.use(config => {
      ...
      // if (getToken() && isToken) {
        config.headers['Cookie'] = 'ASP.NET_SessionId=bcrapdc5trtvhzxxhvrnuc54; CompanyUserInfo=egBoAGEAaQA7AEUAMQAwAEEARABDADMAOQA0ADkAQgBBADUAOQBBAEIAQgBFADUANgBFADAANQA3AEYAMgAwAEYAOAA4ADMARQA7ADIAMAAyADMAMAA4ADAANQAyADMAMwA5ADEAOAA7ADEAOAAxADsAOQA='//`${getToken()}`
      // }
      if (!noLoading) {
        uni.showLoading({
          title: '加载中...'
